...
pygame.display.set_caption("Sorting Master - UI ปรับปรุงใหม่")
...
def draw_button(text, x, y, w, h, color, hover_color):
    ...
    # เอฟเฟกต์ปุ่มตอนเอาเมาส์ไปชี้ (Hover)
...
def main():
    ...
    while running:
        if not game_over:
            ...
            # --- หน้าจอ GAME OVER (UI ใหม่) ---
            overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            overlay.fill((0, 0, 0, 180)) # พื้นหลังดำโปร่งแสง
            ...
            # คะแนนและเวลา (แยกบรรทัด)
            score_res = FONT.render(f"Final Score: {score}", True, WHITE)
            time_res = FONT.render(f"Survival Time: {seconds_passed}s", True, WHITE)
