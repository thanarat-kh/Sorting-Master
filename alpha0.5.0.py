...
pygame.display.set_caption("Sorting Master: High-Score Edition")
...
# --- 2. ระบบจัดการคะแนน (High Score with Time) ---
...
# --- 4. คลาสขยะ (แนวตั้ง: บนลงล่าง) ---
class Garbage:
    def __init__(self):
        # สลับสี: Recycle=Yellow, General=Blue
        ...
        elif self.type == "HAZARDOUS": self.color = HAZARD_RED
...
def main():
    ...
    while running:
        ...
        if state == 'START_MENU':
            ...
            overlay.fill((0, 0, 0, 200)) # พื้นหลังโปร่งแสงให้ Leaderboard เด่น
            ...
            screen.blit(hs_label, (WIDTH//2 - hs_label.get_width()//2, 130))
            header = SMALL_FONT.render("RANK     SCORE      TIME", True, GRAY)
            screen.blit(header, (WIDTH//2 - 130, 175))
            ...
            if state == 'PLAYING':
                ...
                pygame.draw.rect(screen, DARK_GRAY, (WIDTH//2-120, 220, 240, 180), border_radius=20)
                if draw_button("RESUME", WIDTH//2-90, 250, 180, 50, GREEN, (70, 230, 70)):
                    total_paused_time += pygame.time.get_ticks() - pause_start_tick
                    state = 'PLAYING'; pygame.time.delay(200)
                if draw_button("MENU", WIDTH//2-90, 320, 180, 50, BLUE, (100, 100, 255)):
                ...
                screen.blit(FONT.render(f"Score: {score}", True, BLACK), (WIDTH//2-60, 250))
                screen.blit(FONT.render(f"Time: {final_time}s", True, DARK_GRAY), (WIDTH//2-55, 300))
                
                if draw_button("RESTART", WIDTH//2-150, 370, 130, 50, GREEN, (70, 230, 70)):
                    state, score, lives, items, score_saved = 'PLAYING', 0, 5, [], False
                    start_ticks, total_paused_time = pygame.time.get_ticks(), 0; pygame.time.delay(200)
                if draw_button("MENU", WIDTH//2+20, 370, 130, 50, BLUE, (100, 100, 255)):
                    state = 'START_MENU'; pygame.time.delay(200)
