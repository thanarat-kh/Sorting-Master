...
pygame.display.set_caption("Sorting Master - ระบบหยุดเกม (Pause)")
...
def main():
    ...
    # สถานะเกม: 'START_MENU', 'PLAYING', 'PAUSED', 'GAME_OVER'
    state = 'START_MENU'
    ...
    active_item = None
    start_ticks = 0
    paused_ticks = 0 # เก็บเวลาที่ใช้ไปตอนกดหยุด
    final_time = 0
    ...
    while running:
        ...
        if state == 'START_MENU':
            ...
            if draw_button("START GAME", WIDTH//2 - 110, HEIGHT//2 + 20, 220, 50, BLUE, (100, 100, 255)):
                state = 'PLAYING'
            ...
            if draw_button("QUIT GAME", WIDTH//2 - 110, HEIGHT//2 + 90, 220, 50, DARK_GRAY, GRAY):
                running = False
        ...
        # --- หน้าตอนเล่นเกม ---
            ...
            screen.fill(LIGHT_BG)
            # คำนวณเวลา (หักลบเวลาที่เคยหยุดไป)
            ...
            # ปุ่ม Pause มุมขวาบน
            if draw_button("||", WIDTH - 60, 70, 40, 40, DARK_GRAY, GRAY):
                state = 'PAUSED'
                pause_start_time = pygame.time.get_ticks() # เริ่มนับว่าหยุดไปนานแค่ไหน
                pygame.time.delay(200)
        ...
        # --- หน้าหยุดพักเกม (PAUSED) ---
            ...
            screen.blit(pause_title, (WIDTH//2 - pause_title.get_width()//2, HEIGHT//2 - 100))

            if draw_button("RESUME", WIDTH//2 - 100, HEIGHT//2 + 20, 200, 50, GREEN, (70, 230, 70)):
                state = 'PLAYING'
                # บันทึกว่าหยุดไปกี่มิลลิวินาที เพื่อเอาไปลบออกจากเวลาเล่นจริง
                paused_ticks += pygame.time.get_ticks() - pause_start_time
                pygame.time.delay(200)

            if draw_button("MENU", WIDTH//2 - 100, HEIGHT//2 + 90, 200, 50, BLUE, (100, 100, 255)):
                state = 'START_MENU'
                pygame.time.delay(200)
        ...
        # --- หน้าจอจบเกม ---
            ...
            if draw_button("RESTART", WIDTH//2 - 160, HEIGHT//2 + 60, 140, 50, GREEN, (70, 230, 70)):
                score = 0; lives = 5; items = []; paused_ticks = 0
                start_ticks = pygame.time.get_ticks()
                state = 'PLAYING'; pygame.time.delay(200)
            if draw_button("MENU", WIDTH//2 + 20, HEIGHT//2 + 60, 140, 50, BLUE, (100, 100, 255)):
