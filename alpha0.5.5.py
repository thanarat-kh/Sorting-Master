...
def main():
    ...
    active_item = None
    start_ticks = 0
    final_time = 0 # ตัวแปรสำหรับเก็บเวลาตอนจบเกม
    score_saved = False
                    ...
                    for b in bins:
                        if b["rect"].colliderect(active_item.rect):
                            if b["type"] == active_item.type:
                                ...
                                # แก้ไข: ถ้าย้ายลงผิดถัง ไม่หักคะแนนแล้ว หักแค่ชีวิต
                                lives -= 1
                            items.remove(active_item)
                            break
                    if not correct and active_item in items:
                        # แก้ไข: ถ้าปล่อยนอกถัง ไม่หักคะแนน หักแค่ชีวิต
                        items.remove(active_item)
                        lives -= 1
                    active_item = None
        ...
        if state == 'MENU':
            ...
            for item in items[:]:
                ...
                if item.rect.y > HEIGHT:
                    # ปล่อยขยะตกหล่น ไม่หักคะแนน หักแค่ชีวิต
                    items.remove(item)
                    lives -= 1
            ...
            if lives <= 0: 
                state = 'GAMEOVER'
                # แก้ไข: บันทึกเวลาไว้ในตัวแปร final_time ทันทีที่จบเกม
                final_time = (pygame.time.get_ticks() - start_ticks) // 1000

        elif state == 'GAMEOVER':
            # ใช้ final_time แทนการคำนวณตลอดเวลา
            if not score_saved:
            ...
            # โชว์เวลาจาก final_time จะทำให้เวลาไม่เดินขยับต่อ
